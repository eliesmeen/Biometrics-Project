...
import numpy as np 
# Sklearn classifiers - train_test_split - KNeighborsClassifier - OneVsRestClassifier - SVC
...
# Experiment 2: SVM and KNN classifiers - Score based fusion
print("\n[EXPERIMENT 2] KNN+SVC Classifiers")

X_train, y_train = X_clean, y_clean
X_test, y_test = X_beard, y_beard

clf = OneVsRestClassifier(KNeighborsClassifier(n_neighbors=5))
clf_svm = OneVsRestClassifier(SVC(probability=True, kernel='linear'))
clf.fit(X_train, y_train)
...
for i in range(len(y_test)):
    for j in range(len(y_train)):
        try:
            svm_class_idx = list(clf_svm.classes_).index(y_train[j])
            svm_score = svm_probs[i][svm_class_idx]
        except ValueError:
            svm_score = 0  # If class not found - Fallback

        # Fusion of similarity scores (0.5 from KNN, 0.5 from SVM)
        fused_score = 0.5 * similarity_scores[i, j] + 0.5 * svm_score
...
evaluator = Evaluator(
    ...
    genuine_scores=np.array(genuine_scores),
    impostor_scores=np.array(impostor_scores),
    plot_title="Score-Level Fusion: KNN + SVM"
)
...
evaluator.plot_det_curve(FPR, FNR)
evaluator.plot_roc_curve(FPR, TPR)
print("Equal Error Rate (Fused):", evaluator.get_EER(FPR, FNR))